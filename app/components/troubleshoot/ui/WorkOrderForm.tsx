...
// Normalize snake_case API data to camelCase
function normalizeWorkOrderData(input: WorkOrderData): WorkOrderData {
  return {
    workOrderNumber: input.workOrderNumber || input.work_order_number,
    equipmentTag: input.equipmentTag || input.equipment_tag || '',
    equipmentName: input.equipmentName || input.equipment_name,
    priority: input.priority || 'normal',
    workType: input.workType || input.work_type || 'corrective',
    description: input.description || '',
    symptoms: input.symptoms,
    requestedBy: input.requestedBy,
    requestedDate: input.requestedDate,
    targetDate: input.targetDate,
    estimatedHours: input.estimatedHours || input.estimated_hours,
    requiredParts: input.requiredParts || input.required_parts,
    requiredTools: input.requiredTools || input.required_tools,
    safetyRequirements: input.safetyRequirements || input.safety_requirements,
    lotoRequired: input.lotoRequired ?? input.loto_required,
    drawingReference: input.drawingReference || input.drawing_reference,
    notes: input.notes,
  };
}
...
export function WorkOrderForm({ 
  ...
  // Normalize data on init to handle snake_case from API
  const [data, setData] = useState<WorkOrderData>(() => normalizeWorkOrderData(initialData));
  ...
  const defaultPriorityColor = 'bg-blue-500/20 text-blue-400 border-blue-500/30';
        ...
        <div className="p-6 border-b border-white/10 bg-gradient-to-r from-blue-500/10 to-[#71717A]/10">
          <div className="flex items-start justify-between">
            ...
            <div className={cn(
              ...
              priorityColors[data.priority] || defaultPriorityColor
            )}>
