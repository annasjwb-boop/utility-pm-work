...
export function ChecklistCard({ 
  ...
  const handleExport = () => {
    if (cardRef.current) {
      exportToPDF(cardRef.current, {
        title: `Checklist - ${data.title}`,
        filename: `checklist-${data.title?.replace(/[^a-zA-Z0-9]/g, '-') || 'items'}`,
      });
    }
    onExport?.();
  };
  ...
  return (
    <div ref={cardRef} className="rounded-lg bg-[var(--nxb-surface-5)] border border-white/[0.08] overflow-hidden font-['Suisse_Intl',sans-serif]">
      {/* Header */}
